...
const MESSAGES = {
  ...
  INVITE: {
    WELCOME_WITH_INVITER: (member, guild, inviter, code) =>
      `:cute_nhomnhom:Chào mừng ${member} đến với ${guild}!\nĐược mời bởi: ${inviter} (+3 MĐCoin) YAY !!! \nInvite code: \`${code}\``,

    WELCOME_GENERIC: (member, guild) =>
      `:cute_c_e1: Chào mừng ${member} đến với ${guild}!`,

    REWARD_DM: (memberTag, code) =>
      `:cute_c_e3: **Chúc mừng!** Bạn vừa nhận được **3 MĐCoin** vì đã mời ${memberTag} tham gia server!\nInvite code: \`${code}\``
  },
